// https://observablehq.com/@augustocam95/d3-com-crossfilter-e-dc-js@264
...
d3.csv("https://gist.githubusercontent.com/emanueles/d8df8d875edda71aa2e2365fae2ce225/raw/1e949d3da02ed6caa21fe3a7a12a4e5a611a4bab/stocks.csv").then(function(data){
  ...
  let baseLineFace = + data[0].facebook 
  let baseLineGoogle = + data[0].google
  data.forEach(function(d,i){
       d.date = parseDate(d.date)
       d.google = +d.google
       d.facebook = +d.facebook
       d.baseLineFace = + ((d.google - baseLineGoogle)/baseLineGoogle )*100
       d.baseLineGoogle = + ((d.facebook - baseLineFace)/baseLineFace )*100
   })
  return data
})
)});
...
d3.json('https://raw.githubusercontent.com/emanueles/datavis-course/master/assets/files/observable/movies.json').then(function(data){
  ...
  data.forEach(function(d){
       d.budget = +d["Budget_M"];
       d.genre = d["Genre"];
       d.worldwide = +d["Worldwide_Gross_M"]; 
   })
  return data
})
)});
...
facts2.dimension( d => +d["Year"])
  ...
  main.variable(observer("genDim")).define("genDim", ["facts2"], function(facts2){return(
facts2.dimension( d => d["Genre"])
)});
  main.variable(observer("yearGroup")).define("yearGroup", ["yearDim"], function(yearDim){return(
yearDim.group().reduceSum(d => +d["Worldwide_Gross_M"])
)});
  main.variable(observer("genGroup")).define("genGroup", ["genDim"], function(genDim){return(
genDim.group().reduceSum(d => +d["Worldwide_Gross_M"])
)});
  main.variable(observer("by_year")).define("by_year", ["md","container","dc","d3","yearDim","yearGroup"], function(md,container,dc,d3,yearDim,yearGroup)
{
  let view = md`${container('chart3','Total Arrecadado nas Bilheterias por ano em Milhões de Dólares')}`
  ...
  main.variable(observer("by_genre")).define("by_genre", ["md","container","dc","d3","genDim","genGroup"], function(md,container,dc,d3,genDim,genGroup)
{
  let view2 = md`${container('chart4','Total Arrecadado nas Bilheterias por Gênero de 2007 a 2011(em milhões de Dólares)')}`
  let barChart2 = dc.barChart(view2.querySelector("#chart4"))
